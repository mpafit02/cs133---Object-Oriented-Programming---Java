 ...
 * 
 * This class is the main class of the game. It is the menu which interacts with
 * the player. it tells you to choose between some movements to develop your
 * Empire. The main class is using the classes: Building, CoalMine, Desert,
 * Egypt, Fish, Game, Hills, Population, QinDynasty, River, RomanEmpire,
 * Settlement, Strategy, Technology and TTreasury
 * 
 * @author Marios
 */
public class CivilizationGame {
	...
	public static void main(String[] drive) throws Exception {
		System.out.println("Welcome to Civilization! Which civilization would" + " you like to build?\n\n"
				+ "1. Ancient Egypt\n" + "2. Qin Dynasty\n" + "3. Roman Empire\n");
	...
	public static void playEgypt() throws Exception {
		...
		while (!egypt.getStrategy().conqueredTheWorld() && !egypt.getTechnology().hasTechnologyWin()) {
			System.out.println(
					"\n\nWelcome to Egypt - you have a" + " strategy level of " + egypt.getStrategy().getStrategyLevel()
							+ ", an understanding of " + egypt.getTechnology().getUnderstanding()
							+ ", and a build experience of " + egypt.getTechnology().getBuildExperience() + ".\n");
			System.out.println("You have " + egypt.getTreasury().getCoins() + " coins in your treasury, "
					+ egypt.getPopulation().getWarriors() + " warriors, and " + egypt.getPopulation().getCivilians()
					+ " civilians.\n");
	...
	private static void settle(Egypt egypt) {
		System.out.println("Would like to settle a new city?\n" + "1. Yes.\n"
				+ "2. No, I would like to continue building one of my pre-existing" + " cities!\n");
		int move = scan.nextInt();
		scan.nextLine();
		if (move == 1) {
			System.out.println("What would you like to name your new " + "settlement?");
			String settlementName = scan.nextLine();
			Settlement settlement = new Settlement(settlementName);
			if (egypt.settle(settlement)) {
				System.out.println("You have settled the new land of " + settlement.getName());
				build(egypt, settlement);
			} else {
				System.out.println("You have failed to settle. You have too" + " many settlements already.");
			}
		} else if (move == 2) {
			Settlement[] settlements = egypt.getSettlements();
			if (settlements.length > 0) {
				System.out.println("Choose one of these settlements to " + "build upon!");
				for (int i = 0; i < egypt.getNumSettlements(); i++) {
					System.out.println((i + 1) + ". " + settlements[i].getName());
				}
				...
				System.out.println("Sorry, you don't have any pre-existing" + " settlements.");
			}
		} else {
			no();
		}
	...
	private static void build(Egypt egypt, Settlement settlement) {
		...
		String print = "\n";
		if (buildMove == 1) {
			print += egypt.buildPyramid(settlement) ? "Wow, what a beautiful" + " pyramid."
	...
	private static void conquer(Egypt egypt) {
		...
		if (move == 1) {
			if (egypt.getPopulation().getWarriors() > 40) {
				...
				egypt.getPopulation().setWarriors(egypt.getPopulation().getWarriors() - warriorsLost);
	...
	private static void educate(Egypt egypt) {
		...
		System.out.println("\nYour Egyptians are improving their writing! Keep" + " practicin' those hieroglyphics.\n");
	}

	// Explore for Egypt
	private static void explore(Egypt egypt) throws Exception {
		System.out.println("\nEgyptians have two terrains to explore: \n" + "1. Desert\n" + "2. The "
				+ egypt.getRiver().getName() + " River\n");
		System.out.println("Make a choice!\n");

		int move = scan.nextInt();
		if (move == 1) {
			System.out.println("\nThe only point to exploring the Desert is to" + " find treasure, of course!");
			int treasure = egypt.getDesert().findTreasure();
			System.out.println("You explored the Desert and came back with " + treasure + " treasure!\n");
			egypt.getTreasury().earn(treasure);
		} else if (move == 2) {
			gameplayRiver(egypt.getRiver(), egypt.getPopulation(), egypt.getCoalMine());
		} else {
	...
	// Gameplay in Hills
	private static void gameplayHills(Hills hills, Population population, Treasury treasury, CoalMine coalMine) {
		...
		if (move == 1) {
			...
			System.out.println(print + " You found " + goldFound + " gold coins.");
		} else if (move == 2) {
			...
			if (gameFound == null) {
				...
				String replenished = (hills.replenishGame()) ? "Success! We " + "found more Wild Game!\n"
						: "You can only replenish Game" + " once all other Game has been hunted.\n";
				System.out.println(replenished);
			} else {
				if (population.canCook(gameFound, coalMine)) {
					System.out.println("You found some food and successfully" + " fed your citizens!");
				} else {
					System.out.println("You found some food, but you don't have" + " enough coal to cook it.");
				}
			}
		} else if (move == 3) {
	...
	// Gameplay in River
	private static void gameplayRiver(River river, Population population, CoalMine coalMine) throws Exception {
		...
		if (fishFound == null) {
			System.out.println("Unfortunately, it looks like all the Fish have"
					+ " left this terrain.\nNo fear, though! The terrain will " + "replenish itself soon enough.\n");
			String print = (river.replenishFish()) ? "Success! More fish have " + "arrived!\n"
					: "Something went wrong... Not sure where " + "the fish went.\n";
			System.out.println(print);
		} else {
	...
	public static void playQin() throws Exception {
		...
		while (!qin.getStrategy().conqueredTheWorld() && !qin.getTechnology().hasTechnologyWin()) {
			System.out.println("\n\nWelcome to the Qin Dynasty -" + " you have a strategy level of "
					+ qin.getStrategy().getStrategyLevel() + ", an understanding of "
					+ qin.getTechnology().getUnderstanding() + ", and a build experience of "
					+ qin.getTechnology().getBuildExperience() + ".\n");
			System.out.println("You have " + qin.getTreasury().getCoins() + " coins in your treasury, "
					+ qin.getPopulation().getWarriors() + " warriors, and " + qin.getPopulation().getCivilians()
					+ " civilians.\n");
			System.out.println("Welcome to the Qin Dynasty - what would you " + "like to do?\n" + "1. Build my empire\n"
					+ "2. CONQUER\n" + "3. Educate my citizens\n" + "4. Explore my surroundings\n");
	...
	private static void build(QinDynasty qin, Settlement settlement) {
		...
		if (buildMove == 1) {
			...
			print += qin.buildHouse(settlement) ? "Congratulations on your new " + "home!"
					: "You need enough workers AND gold to build this " + "structure.";
		}
		System.out.println(print + "\n");
	...
	private static void conquer(QinDynasty qin) {
		...
		if (move == 1) {
			if (qin.getPopulation().getWarriors() > 40) {
				int warriorsLost = rand.nextInt(30);
				qin.getPopulation().setWarriors(qin.getPopulation().getWarriors() - warriorsLost);
	...
	private static void explore(QinDynasty qin) throws Exception {
		...
		int move = scan.nextInt();
		if (move == 1) {
			gameplayHills(qin.getHills(), qin.getPopulation(), qin.getTreasury(), qin.getCoalMine());
		} else if (move == 2) {
			gameplayRiver(qin.getRiver(), qin.getPopulation(), qin.getCoalMine());
		} else {
			System.out.println("That was not an option, sorry.");
			return;
		}
	...
	private static void educate(QinDynasty qin) {
		...
		System.out.println("\nYour citizens are studying Legalism! Keep " + "producing governmental philosophy!\n");
	}

	// Play in Rome
	public static void playRome() throws Exception {
		RomanEmpire rome = new RomanEmpire();
		while (!rome.getStrategy().conqueredTheWorld() && !rome.getTechnology().hasTechnologyWin()) {
			System.out.println(
					"\n\nWelcome to Rome -" + " you have a strategy level of " + rome.getStrategy().getStrategyLevel()
							+ ", an understanding of " + rome.getTechnology().getUnderstanding()
							+ ", and a build experience of " + rome.getTechnology().getBuildExperience() + ".\n");
			System.out.println("You have " + rome.getTreasury().getCoins() + " coins in your treasury, "
					+ rome.getPopulation().getWarriors() + " warriors, and " + rome.getPopulation().getCivilians()
					+ " civilians.\n");
	...
	private static void build(RomanEmpire rome, Settlement settlement) {
		...
		String print = "\n";
		if (buildMove == 1) {
			print += rome.buildAqueduct(settlement) ? "What an efficient aqueduct!"
					: "You need enough workers AND " + "gold to build this structure.";
		} else if (buildMove == 2) {
			print += rome.buildBathHouse(settlement) ? "You've built a bath house!"
					: "You need enough workers AND gold to build this structure.";
		} else if (buildMove == 3) {
			print += rome.buildVilla(settlement) ? "I wish I lived in a villa " + "this nice."
					: "You need enough workers AND gold to build " + " this structure.";
		}
		System.out.println(print + "\n");
	...
	private static void conquer(RomanEmpire rome) {
		...
		if (move == 1) {
			if (rome.getPopulation().getWarriors() > 40) {
				int warriorsLost = rand.nextInt(30);
				rome.getPopulation().setWarriors(rome.getPopulation().getWarriors() - warriorsLost);
	...
	private static void explore(RomanEmpire rome) throws Exception {
		System.out.println("\nYou have two terrains to explore: \n" + "1. Hills\n" + "2. The "
				+ rome.getRiver().getName() + " River\n");
		System.out.println("Make a choice!\n");

		int move = scan.nextInt();
		if (move == 1) {
			gameplayHills(rome.getHills(), rome.getPopulation(), rome.getTreasury(), rome.getCoalMine());
		} else if (move == 2) {
			gameplayRiver(rome.getRiver(), rome.getPopulation(), rome.getCoalMine());
		} else {
			no();
		}
	...
	private static void educate(RomanEmpire rome) {
		...
		System.out.println("\nYour citizens are philosophizing! Keep " + "questioning.\n");
	}
}